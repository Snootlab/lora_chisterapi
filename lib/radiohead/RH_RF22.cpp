...
bool RH_RF22::init()
{
	...
	switch( _myInterruptIndex ){
		case 0:
			attachInterrupt(_interruptPin, isr0, INT_EDGE_FALLING);
			break;
		case 1:
			attachInterrupt(_interruptPin, isr1, INT_EDGE_FALLING);
			break;
		case 2:
			attachInterrupt(_interruptPin, isr2, INT_EDGE_FALLING);
			break;
		default:
			return false;
	}
...
{
#if (RH_PLATFORM == RH_PLATFORM_RPI)
    RHGenericDriver::setPromiscuous(promiscuous);
#else
...
{
#if (RH_PLATFORM == RH_PLATFORM_RPI)
    RHGenericDriver::setThisAddress(thisAddress);
#else
