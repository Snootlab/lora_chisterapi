...
void RH_RF69::readFifo()
{
	ATOMIC_BLOCK_START;
#if (RH_PLATFORM == RH_PLATFORM_RPI)
	uint8_t buffer[RH_RF69_MAX_ENCRYPTABLE_PAYLOAD_LEN +1];
	uint8_t* pBuffer = buffer;

	uint8_t payloadlen = spiRead(RH_RF69_REG_00_FIFO);
	if( payloadlen <= RH_RF69_MAX_ENCRYPTABLE_PAYLOAD_LEN &&
			payloadlen >= RH_RF69_HEADER_LEN ) {
		spiBurstRead(RH_RF69_REG_00_FIFO, buffer, payloadlen);
		_rxHeaderTo = *pBuffer++;
		// Check addressing
		if (_promiscuous || _rxHeaderTo == _thisAddress ||
				_rxHeaderTo == RH_BROADCAST_ADDRESS ) {
			// Get the rest of the headers
			_rxHeaderFrom  = *pBuffer++;
			_rxHeaderId    = *pBuffer++;
			_rxHeaderFlags = *pBuffer++;
			_bufLen = payloadlen - RH_RF69_HEADER_LEN;
			memcpy(_buf, pBuffer, _bufLen);
			_rxGood++;
			_rxBufValid = true;
		}
	}
...
ATOMIC_BLOCK_START;
#if (RH_PLATFORM == RH_PLATFORM_RPI)
	 uint8_t buffer[RH_RF69_MAX_MESSAGE_LEN+1];
	/* Header */
	 txHeader_t *p_TxHeader = (txHeader_t*) buffer;
	 p_TxHeader->length = len + RH_RF69_HEADER_LEN;
	 p_TxHeader->to = _txHeaderTo;
	 p_TxHeader->from = _txHeaderFrom;
	 p_TxHeader->id = _txHeaderId;
	 p_TxHeader->flags = _txHeaderFlags;
	 /* Data */
	 memcpy(&p_TxHeader->data, data, len);
	 /* Sending */
	 spiBurstWrite(RH_RF69_REG_00_FIFO, buffer, len + RH_RF69_HEADER_LEN + 1)
#else
    digitalWrite(_slaveSelectPin, LOW);
